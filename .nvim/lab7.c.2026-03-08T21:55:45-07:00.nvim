...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  int newResult = *result_count;
  output[newResult].line_numbers[output[newResult].count] = input->line_number;
  output[newResult].doubled_value = input->doubled_value;
  output[newResult].count++;
  (*result_count)++;
}
