...
int main() {
  ...
  FILE *path = fopen("public_key.pem", "r");
  EVP_PKEY *pubkey = PEM_read_PUBKEY(path, NULL, NULL, NULL);
...
i        -1: Message is does not match signature
*/
int verify(const char *message_path, const char *sign_path, EVP_PKEY *pubkey) {
#define MAX_FILE_SIZE 512
  ...
  int mrBytes = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  int srBytes = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);

  EVP_MD_CTX *ctx = EVP_MD_CTX_new();

  int init = EVP_DigestVerifyInit(ctx, NULL, EVP_sha256(), NULL, pubkey);
  int update = EVP_DigestVerifyUpdate(ctx, message, mrBytes);
  int fin = EVP_DigestVerifyFinal(ctx, signature, srBytes);
